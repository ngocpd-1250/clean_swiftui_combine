...
open class APIBase {
        ...
        return response
            .tryMap { apiResponse -> APIResponse<T> in
                do {
                    ...
                    return APIResponse(header: apiResponse.header, data: decodedObject)
                } catch {
        ...
        let urlRequest = preprocess(input)
            .handleEvents(receiveOutput: { [unowned self] input in
                if logOptions.contains(.request) {
                    print(input.description(isIncludedParameters: logOptions.contains(.requestParameters)))
                }
            })
            ...
            .tryCatch { [unowned self] error -> AnyPublisher<APIResponse<U>, Error> in
                return try handleRequestError(error, input: input)
            }
    ...
    open func handleRequestError<U: JSONData>(_ error: Error,
                                              input _: APIInputBase) throws -> AnyPublisher<APIResponse<U>, Error> {
        throw error
    }
